...
function appliquerCouleurMarqueBlanche() {
    ...
    // Boutons et bordures dynamiques
...
function chargerHistorique() {
    ...
    // CORRECTION DES BOUTONS (On passe bien l'event pour stopper la propagation)
            ...
            <tr class="history-row">
                ...
                <td style="text-align: right;">
                    <div style="display:flex; justify-content:flex-end; gap:5px;">
                        <button type="button" class="btn-voir" onmouseover="this.style.color='${agenceCouleur}'; this.style.borderColor='${agenceCouleur}'" onmouseout="this.style.color='#0b1a14'; this.style.borderColor='#ced4da'" onclick="event.stopPropagation(); voirPDFDirect(${realIndex});">
                        ...
                        <button type="button" class="btn-pdf" style="background-color: ${agenceCouleur}; color: #fff;" onclick="event.stopPropagation(); telechargerDirect(${realIndex});">
...
// ACTION : BOUTON VOIR (OUVRE LE PDF DIRECTEMENT SANS BUG)
function voirPDFDirect(index) {
...
// ACTION : BOUTON PDF (TÉLÉCHARGEMENT DIRECT SANS BUG)
...
function lancerDemo() {
    ...
    donneesAudit = {
        ...
        impact_marche: "Secteur provincial actif. Tension économique constatée sur les devis liés aux artisans certifiés RGE.",
        ...
        diagnostics: [
            {titre: "Électricité (Sécurité)", cout: 8128, loi: "Norme NF C 15-100", detail: "Défaut de mise à la terre ou matériel ancien identifié.", action: "Mise en sécurité du tableau électrique."},
...
// LE NOUVEAU PDF : STYLE "CABINET DE CONSEIL" - EPURE ET DENSE (SANS VERT FLUO)
function exporterPDF(action = 'download') {
    ...
    // LOGO CADRÉ PARFAITEMENT ALIGNÉ A GAUCHE, DETAILS A DROITE
    ...
    try {
        let docDefinition = {
            ...
            content: [
                ...
                { text: 'Synthèse Exécutive : Ce document regroupe les données extraites du Dossier de Diagnostic Technique (DDT). Il présente une évaluation objective des coûts de remise aux normes pour sécuriser et justifier la transaction immobilière face aux exigences réglementaires et bancaires.', fontSize: 10, color: '#555', alignment: 'justify', margin: [0, 0, 0, 40], italics: true },
        ...
        if (action === 'open') {
            pdf.open();
            if(btn) {
                btn.innerText = "Télécharger le rapport PDF Officiel";
                btn.style.opacity = '1';
            }
        } else {
